...
# For observability - removed LangFuse for HuggingFace compatibility
import yaml
import logging
import datetime
...
# LangFuse components removed for HuggingFace compatibility
...
Please provide a comprehensive analysis with specific suggestions for improvement."""
    ...
    def _create_graph_instance(self, session_id: str, pdf_data: Dict = None):
        """Create graph instance for processing"""
        # Return basic graph without observability
...
Please provide a comprehensive analysis with specific suggestions for improvement."""
    ...
    def analyze_resume_text(self, resume_text: str, session_id: str = None) -> Dict:
        ...
        try:
            ...
            graph_instance = self._create_graph_instance(session_id, None)
            
            # Run the analysis
            logger.info(f"Starting resume analysis for session: {session_id}")
            final_state = graph_instance.invoke(
                initial_state,
    ...
    def analyze_resume_with_pdf_data(self, resume_text: str, pdf_data: Dict, session_id: str = None) -> Dict:
        ...
        try:
            ...
            # Create graph instance
            graph_instance = self._create_graph_instance(session_id, pdf_data)
    ...
    def analyze_resume_with_prompt(self, pdf_data: Dict, prompt_key: str, session_id: str = None) -> Dict:
        ...
        try:
            ...
            # Extract author name for metadata
            author_name = self._extract_author_name(pdf_data)
...
Based on the analysis above, please provide optimized resume content following the guidelines in the prompt."""
    ...
    def _extract_author_name(self, pdf_data: Dict) -> str:
        """Extract author name from PDF data"""
        try:
            if pdf_data and "pdf_info" in pdf_data:
                author = pdf_data["pdf_info"].get("author", "")
                if author and author.lower() not in ['unknown', '', 'none', 'null']:
                    # Clean the name for use in metadata
                    import re
                    clean_name = re.sub(r'[^a-zA-Z0-9_]', '_', author.strip())
